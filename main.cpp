...
//Declaring  as global variable for p and t 
Pressure p;
Temperature t;
...
HEPTA_SENSOR sensor(p17,
                  ...
                  I2C i2c(p28, p27);   // pressure sensor from lab7


HEPTA_COM com(p9,p10,9600);
MPL3115A2 sensor1(&i2c); // linked with folder
...
int main() {
    ...
    for(int i = 0; i < 100; i++) {
        ...
        sensor1.setOffsetTemperature(20);
        sensor1.setOffsetPressure(-32);
        Temperature t;
        
        Pressure p;
       
        sat.printf("** MPL3115A2 SENSOR **\r\n");
        
        sensor1.readTemperature(&t);
        
        sensor1.setModeStandby();
        sensor1.setModeBarometer();
        sensor1.setModeActive();
        sensor1.readPressure(&p);
        
        //Transmitting HK data to Ground Station(GS)
        com.printf("HEPTASAT::Condition = %d, Time = %f [s], batVol = %.2f [V],Temp = %.2f, P=%f [C]\r\n",flag,sattime.read(),batvol,temp,p.print());
        wait_ms(1000);
    ...
    sat.printf(" Temp: %sdegF, Pressure: %sPa\r\n", t.print(), p.print());
}
